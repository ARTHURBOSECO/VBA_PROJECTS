...
Sub MONTAR_TABELAS()
    ...
    Dim CABEÇALHO_1 As Range
    Dim CABEÇALHO_2 As Range
    Dim CABEÇALHO_FOL As Range
    Dim CABEÇALHO_FER As Range
    ...
    Set CABEÇALHO_1 = CAB_CAL.Range("B3:R4")
    Set CABEÇALHO_2 = CAB_CAL.Range("B7:R8")
    Set CABEÇALHO_FOL = CAB_CAL.Range("B11:J12")
    Set CABEÇALHO_FER = CAB_CAL.Range("B15:D16")
...
For Each celula In rng
    If IsEmpty(celula.Offset(0, -1)) And celula.Value = "FÉRIAS" Then
        contador = contador + 1
    End If
Next celula
...
For Each celula In rng
    If celula.Offset(0, -1) = "FÉRIAS" And _
    IsEmpty(celula.Value) Then
        contador = contador + 1
    End If
Next celula
...
Sub MONTAR_ESCALA()
    ...
    For linha_1 = linha_I_1 To linha_F_1
        For coluna = 6 To 21
            ...
            Cells(3, coluna).Value <= ESCALAS.Range("Y" & linha_1 + ultimaLinha + 1 + Const1).Value Then
                Cells(linha_1, coluna).Value = "FÉRIAS"
            End If
        Next coluna
    Next linha_1
    ...
    For linha_2 = linha_I_2 To linha_F_2
        For coluna = 6 To 21 - VAC
            ...
            Cells(Cab_Lin, coluna).Value <= ESCALAS.Range("Y" & linha_2).Value Then
                Cells(linha_2, coluna).Value = "FÉRIAS"
            End If
        Next coluna
    Next linha_2
    ...
    '# CRIAÇÃO DO ARRAY DOS BOXES #'
    
    Dim arr() As String ' Array dinâmico para armazenar os valores concatenados

    ' Redimensiona o array dinamicamente com base na última linha
    ...
    arrayIndex = LBound(arr) ' Começa do primeiro elemento do array
    
    '# CÓPIA DO ARRAY DOS BOXES #'
    
    Dim arrC() As String ' Array dinâmico para armazenar os valores concatenados
    ...
    arrayIndexC = LBound(arrC) ' Começa do primeiro elemento do array
            ...
            contadorT = 0
                ...
                For Each Cel In rng1
                    If IsEmpty(Cel.Value) Then
                        ...
                        ESCALAS.Cells(Cel.Row, Cel.Column + 1).Value = "FÉRIAS" Then
            ...
            KI = UBound(linhasVazias1)
                If KI > 0 Then
                            ...
                            For Each Cel In rng2
                                If IsEmpty(Cel.Value) Then
                                    If ESCALAS.Cells(Cel.Row, Cel.Column + 1).Value = "FOLGA" Or _
                                    ESCALAS.Cells(Cel.Row, Cel.Column + 1).Value = "FÉRIAS" Or _
    ...
    '# FORMATANDO FOLGA, FÉRIAS, POSTO E APOIO #'
    ...
    For i = 6 To 21
        For j = 1 To 200
            If ESCALAS.Cells(j, i).Value = "FOLGA" Then
                ...
                Application.CutCopyMode = False
            End If
            If ESCALAS.Cells(j, i).Value = "FÉRIAS" Then
    ...
    '# CONTAGEM DOS TÉCNICOS #'
    
    ESCALAS.Cells(linha_F_1, 5).Value = "TÉC. ATIVOS"
    ...
    For Col = Col_I To Col_F_1
        If IsEmpty(ESCALAS.Cells(linha_F_1, Col).Value) Then
            ...
            - WorksheetFunction.CountIf(ESCALAS.Range(Cells(linha_I_1, Col), Cells(linha_F_1 - 1, Col)), "FÉRIAS")
        End If
        ESCALAS.Range("B2").Copy
    ...
    ESCALAS.Cells(linha_F_2, 5).Value = "TÉC. ATIVOS"
    ...
    For Col = Col_I To Col_F_2
        If IsEmpty(ESCALAS.Cells(linha_F_2, Col).Value) Then
            ...
            - WorksheetFunction.CountIf(ESCALAS.Range(Cells(linha_I_2, Col), Cells(linha_F_2 - 1, Col)), "FÉRIAS")
        End If
        ESCALAS.Range("B2").Copy
